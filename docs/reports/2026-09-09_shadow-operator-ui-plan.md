...
| F7 | The lock file had no staleness handling, no realpath keying, and contradicted `_require_out`'s ordering. | Fully specified in 2h: pid + start time, `os.kill(pid, 0)`, auto-reclaim, `--force` named in the refusal, realpath keying, startup creates the directory. Four new tests. |
