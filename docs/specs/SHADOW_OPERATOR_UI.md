...
18. `model_version` tracks the artifacts directory, and `state.json` is written by every
    route whose CLI equivalent writes it.
    *(Extended. The original wording was satisfiable without `_write_state`, which
    `status()` reads exclusively — omitting it left `GET /api/status` permanently empty.)*
...
24. *(Strengthened.)* Client-side entry pre-validation agrees with `parse_entries` on a
    shared vector set —
...
concurrently across dates; the two frontend test files the plan promised and did not deliver
(`stamps`, entry order) exist as `web/src/routes/Today.test.tsx`; and the `Today` crash the
