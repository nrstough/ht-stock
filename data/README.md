...
`store_synth.csv` — 9,864 rows, one per item per day, 3 years (2023-01-01..2025-12-31),
9 prepared-food items. Every parameter lives in `sim/params.py`.

These bytes are frozen provenance, so the generator refuses to write this path:

```bash
# from the repo root
python -m sim.generate --out .rehearsal/synth.csv
cmp data/store_synth.csv .rehearsal/synth.csv
```

The seed is fixed, so that `cmp` is silent — regenerating reproduces this file byte for
byte. `python -m sim.generate` with no `--out` exits 1 and says so; `--force-frozen` is the
only way to replace it, and doing so invalidates `results/results.json` and every dollar
figure in the proposal.
...
So `ht.schema.conform(pd.read_csv("data/store_synth.csv"))` passes untouched: three of the four
columns above are inserted at their defaults, `row_status` is *derived* from `is_closed`
(`closed` where the store was shut, `ok` otherwise, because `is_closed == int(row_status !=
"ok")`), and the three simulation-only columns are dropped. That is
